...
class PixabayController extends Controller
{
	...
	private function _searchUrl($query, $page = 1) {
		//Build API URL

		$apiKey = PixabayPlugin::$plugin->settings->apiKey;
		return 'https://pixabay.com/api/?key=' . urlencode($apiKey) . '&q=' . urlencode($query) . '&page=' . ((int)$page) . '&per_page=20';
	}
	
	
	private function _getCache($query, $page) {
		...
		$cacheFile = $cacheDir . DIRECTORY_SEPARATOR . base64_encode(strtolower(trim($query))) . ((int)$page) . '.json';
	...
	private function _saveCache($query, $page, $data) {
	...
	public function actionSearch() {
		...
		$cachedData = $this->_getCache($query, $page);
		if ($cachedData)
			return $this->asJson(['success' => true, 'data' => $cachedData]);
		
		$url = $this->_searchUrl($query, $page);
		...
		if ($success)
			$this->_saveCache($query, $page, $pixabayData);
